...
#reader = csv.reader(open("fw_ready.csv", "r"), delimiter=',')
#writer = csv.writer(open("fw_ready_semi.csv", 'w'), delimiter=';')
...
def split_file(filename, pattern, size):
    ...
    with open("fw_ready_semi.csv", 'rb') as f:
        header = next(f)
...
if __name__ == '__main__':
    split_file('data.csv', 'store_{0:03d} deployment.csv', 20)
    #split_file('data.csv', 'store_deployment.csv', 20)
