...
sh = xlrd.open_workbook('MPLS IP STORE ALLOCATION V_16.18.xlsx').sheet_by_index(0)
plik = open("switchip.csv", 'w')

try:
    for rownum in range(sh.nrows):
        plik.write(str(sh.cell(rownum, 2).value)+","+
        str(sh.cell(rownum, 3).value)+","+
        str(sh.cell(rownum, 52).value)+","+
        str(sh.cell(rownum, 53).value)+ "\n")

finally:
    plik.close()
# replacing cell D1 with "Gateway" value
import csv
r = csv.reader(open('switchip.csv')) # Here your csv file
lines = list(r)
lines[0][3] = 'Switch2'
writer = csv.writer(open('SwitchIP_'+dt+'.csv', 'w'))
